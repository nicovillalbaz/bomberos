...
export const createGuardia = async (guardia: GuardiaCreate) => {
  ...
  const { data, error } = await supabase.from('guardias').insert(guardiaData).select().single()
  if (error) throw error
  if (miembros && miembros.length > 0) {
    const miembrosData = miembros.map(m => ({ guardia_id: (data as any).id, miembro_id: m }))
    await supabase.from('guardia_miembros').insert(miembrosData)
  }
  return data
}

export const markAsistencia = async (asistencia: AsistenciaCreate) => {
  const { data, error } = await supabase.from('asistencia').insert(asistencia).select().single()
  if (error) throw error
  return data
}
