...
export const createServicio = async (servicio: ServicioCreate) => {
  ...
  const { data, error } = await supabase.from('servicios').insert(servicioData).select().single()
  if (error) throw error
  if (personal && personal.length > 0) {
    const personalData = personal.map(p => ({ servicio_id: (data as any).id, ...p }))
    await supabase.from('servicio_personal').insert(personalData)
  }
  return data
}
...
export const getTiposServicio = async () => {
  const { data, error } = await supabase.from('tipo_servicio').select('*').eq('activo', true)
  if (error) throw error
  return data
}

export const getSubtiposServicio = async (tipoServicioId?: string) => {
  let query = supabase.from('subtipo_servicio').select('*, tipo_servicio:tipo_servicio_id(*)').eq('activo', true)
