...
export default function Guardias() {
  ...
  const getShiftTimes = (tipo: 'voluntaria' | 'rentada', date: string) => {
    const day = new Date(date).getDay()
    if (tipo === 'rentada') {
      return day === 6 ? { hora_inicio: '07:00', hora_fin: '15:00' } : { hora_inicio: '07:00', hora_fin: '18:00' }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const times = getShiftTimes(form.tipo, form.fecha)
    await createGuardia({ ...form, ...times })
