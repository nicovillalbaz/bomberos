...
export default function Moviles() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowForm(false); setEditing(null); setForm({ nombre:'', dominio:'', marca:'', modelo:'', anio:0, tipo:'camion', estado:'disponible' }); load()
  }

  const handleEdit = (v: Vehiculo) => { setEditing(v); setForm({ nombre:v.nombre, dominio:v.dominio||'', marca:v.marca||'', modelo:v.modelo||'', anio:v.anio||0, tipo:v.tipo, estado:v.estado }); setShowForm(true) }
